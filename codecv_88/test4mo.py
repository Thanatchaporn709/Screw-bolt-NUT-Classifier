...
def save_to_json(original_image, object_type, width, length, save_directory, processed_image_path):
    data = {
        "image_path": os.path.abspath(processed_image_path),  # Ensure absolute path
        "type": object_type,
        "width": str(width),
        "length": str(length)
    }

    output_path = os.path.join(save_directory, "output.json")
...
def process_image(image_path):
    ...
    for indexObj in range(len(contours)):
        rect = cv2.minAreaRect(contours[indexObj])
        ...
        check = check_nut(img, rect)  # This function decides if it's a Nut or needs further classification
        
        # **💡 Your Code Starts Here**
        if check == "Either Screw or Nut":
            size, length = find_dimension(width, height)  # Compute size and length
            result = classify_screw_or_bolt(img, rect)  # Classify as Screw or Bolt
        else:
            size = find_dimension_nut(width, height)  # Compute Nut size
            length = 0  # Nut doesn't have a length
            result = "Nut"
        # **💡 Your Code Ends Here**
        ...
        # **📌 Save JSON Output (so C# Can Read It)**
        if success:
            print(f"✅ Processed image saved at: {processed_image_path}")
            save_to_json(image_path, result, size, length, save_directory, processed_image_path)  # Save JSON Output
        else:
