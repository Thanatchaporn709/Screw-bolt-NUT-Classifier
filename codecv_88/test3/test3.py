...
def find_dimension_nut(width, height):
  pixel = 38/152
  diameter1 = width * pixel
  diameter2 = height * pixel
  return (diameter1 + diameter2)/2

def classify_screw_or_bolt(image):
...
def prepare_image(RGB, rect):
  ...
  ret,bw2 = cv2.threshold(check_img,150,255,cv2.THRESH_BINARY_INV)
  ...
  if np.sum(cropped_image_1 == 255) < np.sum(cropped_image_2 == 255):
    ...
    print(sum(cropped_image_1.flatten()))
    print(sum(cropped_image_2.flatten()))
    print("True")
  ...
  result = classify_screw_or_bolt(transparent_canvas)
  return result
