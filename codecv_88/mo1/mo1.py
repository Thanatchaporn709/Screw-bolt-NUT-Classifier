...
output_json_path = r"C:/Users/User/source/repos/CV88/output88.json"  # เส้นทางสำหรับบันทึก JSON
...
def find_dimension(width, height):
  pixel = 0.145
  if width < height:
    size = round(((width * pixel) / 2) + 1, 2)
    length = round(height * pixel - 4, 2)
  else:
    size = round(((height * pixel) / 2) + 1, 2)
    length = round(width * pixel - 4, 2)
  return size, length

def find_dimension_nut(width, height):
  pixel = 0.142
  diameter1 = round(((width * pixel) / 2) + 1, 2)
  diameter2 = round(((height * pixel) / 2) + 1, 2)
  real_diameter = round((diameter1 + diameter2)/2, 2)
...
def process_image(image_path):
    ...
    ret, bw1 = cv2.threshold(gray, 40, 255, cv2.THRESH_BINARY_INV)
    ...
    classification_results = []  # รายการสำหรับเก็บผลลัพธ์การจำแนกประเภท
    ...
    if contours:  # Ensure at least one contour is found
        ...
        for indexObj in range(len(contours[0])):
            ...
            if width < height:
                size = round(width * pixel, 2)
                length = round(height * pixel, 2)
            else:
                size = round(height * pixel, 2)
                length = round(width * pixel, 2)
            ...
            if check == "Either Screw or Nut":
                ...
                classification_results.append({"type": result, "size": size, "length": length})  # บันทึกผลลัพธ์
            else:
                size = find_dimension_nut(width, height)
                draw_rotated_text(img, "Nut", box[2], angle)
                print("Diameter of Nut:", size)
                classification_results.append({"type": "Nut", "size": size})  # บันทึกผลลัพธ์
    ...
    # ✅ Ensure each classification result includes the image path
    for result in classification_results:
        result["image_path"] = processed_image_path  # Add the processed image path to each entry

    # บันทึกผลลัพธ์ในไฟล์ JSON
    with open(output_json_path, 'w') as json_file:
        json.dump(classification_results, json_file, indent=4)
        print(f"✅ Classification results saved at: {output_json_path}")
...
print("Capturing an image... Press 'Space' to save and process the image. Press 'q' to quit.")
...
while True:
    ...
    # Check for 'Space' key to save the image
    if cv2.waitKey(1) & 0xFF == ord(' '):  # Spacebar pressed
        ...
        if success:
            ...
            processed_path = process_image(save_path)  # Process the saved image
        else:
